    ...
    <div  id='Contact'  className=' flex justify-center max-md:flex-col  max-lg:items-start max-lg:flex-col overflow-hidden items-center p-[5%] gap-[20%] bg-black '>
        <form onSubmit={fetchApi} method='post'>
            <div className=' flex shape flex-col justify-center items-start text-start gap-5  '>
                ...
                <input onChange={(e)=> setName(e.target.value)} name='name' value={name} className='w-[35vw]  max-lg:w-[80vw] max-sm:w-[90vw] bg-[#1E1E1E] p-3 make border-none text-white  rounded-[5px]  hover:border-b'  type="text" id="name"  placeholder="Name" required="" data-error="Please enter your name"></input>
                <input  onChange={(e)=> setEmail(e.target.value)} name='email' value={email}  className='w-[35vw]  max-lg:w-[80vw] max-sm:w-[90vw] bg-[#1E1E1E] p-3 make border-none text-white  rounded-[5px]  hover:border-b      ' type="email" required  placeholder='Email'  id="email" />
                <input  onChange={(e)=> setSubject(e.target.value)} name='subject' value={subject}  className='w-[35vw]  max-lg:w-[80vw] max-sm:w-[90vw]  bg-[#1E1E1E] p-3 make border-none text-white  rounded-[5px]  hover:border-b  ' type="text" placeholder='Subject' id="" />
                <div className='flex flex-col gap-4 text-start '>
                    <textarea onChange={(e)=> setMessage(e.target.value)} name='message' value={message} className='  max-lg:w-[80vw] max-sm:w-[90vw]  w-[35vw] bg-[#1E1E1E] rounded-[5px]  text-[grey] p-3  h-[30vh]  border-1 ' placeholder='Message' id=""></textarea>
                    <button  className='p-2 bg-[#cF1F1F] text-[#E9E9E9] font-mono max-sm:w-[25vw]  max-lg:w-[30vw]    w-[10vw] '>{sending ? 'sending...' : 'Send' } </button>
                </div>
           </div>
           <ToastContainer />
        </form>
        <div className=' flex flex-col max-md:justify-start  max-sm:items-start max-sm:text-start max-sm:mr-[%]  gap-5 p-2 '>
            ...
            <div className='flex flex-col gap-3 '>
                <div className='flex gap-3 items-center'>
                   <MdLocationOn className='text-[#cF1F1F] animate-bounce ' size={30} />
            ...
            <div className='flex flex-col  gap-3'>
                <div className='  gap-3' >
                    <a className='font-mono flex gap-3 items-center' href='https://wa.me/2347066214165'><FaWhatsapp className='text-[#CF1F1F]  animate-bounce  hover:text-[white]' size={40}  />
